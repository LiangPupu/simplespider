...
class Qiubai():
    def __init__(self):
        self.headers = {"User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_5) AppleWeb\
    ...
    def get_content(self):
        while self.html_queue.not_empty:
            ...
            total_div = html.xpath("//div[contains(@class, 'recommend-article')]//li[contains(@class, 'item')]")
            items = []
            for i in total_div:
                author_img = i.xpath(".//a[contains(@class, 'recmd-user')]//img/@src")
                author_img = "https:" + author_img[0] if len(author_img) > 0 else None
                author_name = i.xpath(".//a[contains(@class, 'recmd-user')]//span/text()")
                author_name = author_name[0] if len(author_name) > 0 else None
                author_href = i.xpath(".//a[contains(@class, 'recmd-user')]/@href")
    ...
    def run(self):
        ...
        thread_save = threading.Thread(target=self.sava_item)
